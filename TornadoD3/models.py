...
class saggest (MySQLModel):
    name = peewee.ForeignKeyField(rel_model=User, to_field=User.id)
    email = peewee.ForeignKeyField(rel_model=User, to_field=User.id)
    message = peewee.ForeignKeyField(rel_model=Buy, to_field=Buy.id)
